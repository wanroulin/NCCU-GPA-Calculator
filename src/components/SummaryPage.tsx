...
import { calculateSemesterGPA, GradeStats, Course } from '../utils/gpaCalc';
import { GRADE_COLORS } from '../utils/constant';
import '../styles/components.css';

interface SummaryPageProps {
  gradesData: { [key: string]: Course[] };
  cumulative: GradeStats;
  gradeDistribution: { [key: string]: number };
}

interface SemesterTrendData {
  semester: string;
  gpa: number;
}
...
const SummaryPage: React.FC<SummaryPageProps> = ({ gradesData, cumulative, gradeDistribution }) => {
  // 各學期 GPA 趨勢資料
  const semesterTrend: SemesterTrendData[] = Object.keys(gradesData).sort().map(semester => ({
    semester,
    gpa: parseFloat(calculateSemesterGPA(gradesData[semester]).gpa)
  }));

  // 等級分布圓餅圖資料
  const pieData: PieDataItem[] = Object.entries(gradeDistribution).map(([grade, count]) => ({
    name: grade,
    value: count
  }));

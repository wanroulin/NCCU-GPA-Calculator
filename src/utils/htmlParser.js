 ...
 */
export const parseNCCUGradeHTML = async (htmlContent) => {
  ...
  tables.forEach(table => {
    ...
    rows.forEach(row => {
      ...
      if (cells.length >= 8) {
        ...
        const credits = parseFloat(cells[5].textContent.trim());
 ...
 * @param {string} scoreText - 分數文字
 * @returns {boolean}
 */
const shouldSkipCourse = (course, scoreText) => {
  // 停修課程
...
/**
 * 判斷課程類型
 * @param {string} typeText - 類型文字
 * @returns {string} '必' | '群' | '選'
 */
const getCourseType = (typeText) => {
